...
void Logger::writeToFile(const UnicodeString &logDir) {

	if (mainSession && mainSession->getAppSettings().getEnableLogging()) {

		UnicodeString dirPath = FileUtils::createProjectSubDirPath(logDir);
...
UnicodeString Logger::archiveLogFiles()
{
	...
	UnicodeString command =	FileUtils::createProjectSubDirPath("Archiver") + "Win32\\Debug\\Archiver.exe tar -czf " + \
	FileUtils::createProjectSubDirPath("Log") + "archive_" + TimeManager::getCurrentDate() +  "_" + index + ".tgz -C " + \
	FileUtils::createProjectSubDirPath("Log") + " " + TextUtils::vectorToString(matchingLogs);
